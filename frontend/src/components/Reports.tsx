...
export default function Reports() {
  ...
  const paymentData = data
    ? Object.entries(data.revenue.byPaymentMethod).map(([method, { amount, count }]) => ({
  ...
  return (
    <div className="page">
      <div className="page-header">
        <h2>Reports</h2>
        <p>Monthly financial overview for {MONTH_NAMES[month - 1]} {year}</p>
      </div>
        ...
        <>
          <div className="reports-summary">
            <div className="report-card report-card--revenue">
              ...
              <span className="report-card__sub">{data.revenue.transactionCount} transactions · {data.revenue.discounts.toLocaleString()} RWF discounts</span>
            </div>
          ...
          {data.revenue.transactionCount === 0 && data.expenses.total === 0 && (
            <p className="empty-state">No data found for {MONTH_NAMES[month - 1]} {year}.</p>
          )}
        </>
      )}
    </div>
